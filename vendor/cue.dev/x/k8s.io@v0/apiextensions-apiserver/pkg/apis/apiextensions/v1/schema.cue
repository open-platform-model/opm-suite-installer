...
#CustomResourceDefinitionCondition: {
	...
	// set based upon. For instance, if .metadata.generation is currently 12, but
	// the .status.conditions[x].observedGeneration is 9, the condition is out of
...
#CustomResourceDefinitionSpec: {
	...
	// the OpenAPI schema should be preserved when persisting to storage.
...
#CustomResourceDefinitionVersion: {
	...
	// version is deprecated and recommends use of the newest served version of
	...
	// selectableFields specifies paths to fields that may be used as field
...
#CustomResourceSubresourceScale: {
	...
	// path must be a string field (not a complex selector struct) which contains a
...
#JSONSchemaProps: {
	...
	"default"?: #JSON
	"definitions"?: [string]:  #JSONSchemaProps
	"dependencies"?: [string]: #JSONSchemaPropsOrStringArray
	"description"?:      string
	"enum"?:             [...#JSON]
	"example"?:          #JSON
	...
	// parsed by Golang net/mail.ParseAddress - hostname: a valid representation
	// for an Internet host name, as defined by RFC 1034, section 3.1 [RFC1034]. -
	// ipv4: an IPv4 IP as parsed by Golang net.ParseIP - ipv6: an IPv6 IP as
	// parsed by Golang net.ParseIP - cidr: a CIDR as parsed by Golang
	// net.ParseCIDR - mac: a MAC address as parsed by Golang net.ParseMAC - uuid:
	// an UUID that allows uppercase defined by the regex
	...
	// isbn13: an ISBN13 number string like "978-0321751041" - creditcard: a credit
	// card number defined by the regex
	// ^(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|6(?:011|5[0-9][0-9])[0-9]{12}|3[47][0-9]{13}|3(?:0[0-5]|[68][0-9])[0-9]{11}|(?:2131|1800|35\\d{3})\\d{11})$
	// with any non digit characters mixed in - ssn: a U.S. social security number
	// following the regex ^\\d{3}[- ]?\\d{2}[- ]?\\d{4}$ - hexcolor: an
	...
	// full-date in RFC3339 - duration: a duration string like "22 ns" as parsed by
	// Golang time.ParseDuration or compatible with Scala duration format -
	...
	// x-kubernetes-int-or-string specifies that this value is either an integer or
	// a string. If this is true, an empty type is allowed and type as child of
	...
	// pruning fields which are not specified in the validation schema. This
	// affects fields recursively, but switches back to normal pruning behaviour if
	// nested properties or additionalProperties are specified in the schema. This
	// can either be true or undefined. False is forbidden.
	"x-kubernetes-preserve-unknown-fields"?: bool

	// x-kubernetes-validations describes a list of validation rules written in the
...
#SelectableField: {
	// jsonPath is a simple JSON path which is evaluated against each custom
	// resource to produce a field selector value. Only JSON paths without the
	// array notation are allowed. Must point to a field of type string, boolean or
	// integer. Types with enum values and strings with formats are allowed. If
	// jsonPath refers to absent field in a resource, the jsonPath evaluates to an
	// empty string. Must not point to metdata fields. Required.
	"jsonPath"!: string
}
...
#ValidationRule: {
	...
	// location of this x-kubernetes-validations extension in the schema and refer
	// to an existing field. e.g. when validation checks if a specific attribute
	// `foo` under a map `testMap`, the fieldPath could be set to `.testMap.foo` If
	// the validation checks two lists must have unique attributes, the fieldPath
	...
	// field currently. Refer to [JSONPath support in
	...
	// URL with the host matching spec.host"
	"message"?: string
	...
	// results in a runtime error, the runtime error is logged, and the validation
	// failure message is produced as if the messageExpression field were unset. If
	// messageExpression evaluates to an empty string, a string with only spaces,
	// or a string that contains line breaks, then the validation failure message
	// will also be produced as if the messageExpression field were unset, and the
	// fact that messageExpression produced an empty string/string with only
	// spaces/string with line breaks will be logged. messageExpression has access
	// to all the same variables as the rule; the only difference is the return
	// type. Example: "x must be less than max ("+string(self.max)+")"
	"messageExpression"?: string

	// optionalOldSelf is used to opt a transition rule into evaluation even when
	// the object is first created, or if the old object is missing the value.
	//
	...
	//
	// You may check for presence of oldSelf using `oldSelf.hasValue()` and unwrap
	// it after checking using `oldSelf.value()`. Check the CEL documentation for
	...
	// reason provides a machine-readable validation failure reason that is returned
	// to the caller when a request fails this validation rule. The HTTP status
	// code returned to the caller will match the reason of the reason of the first
	// failed validation rule. The currently supported reasons are:
	// "FieldValueInvalid", "FieldValueForbidden", "FieldValueRequired",
	// "FieldValueDuplicate". If not set, default to use "FieldValueInvalid". All
	// future added reasons must be accepted by clients when reading this value and
	...
	// of the object are field selectable via `self.field` and field presence can
	// be checked via `has(self.field)`. Null valued fields are treated as absent
	// fields in CEL expressions. If the Rule is scoped to an object with
	...
	// `self.all(...)`. If the Rule is scoped to an array, the elements of the
	// array are accessible via `self[i]` and also by macros and functions. If the
	// Rule is scoped to a scalar, `self` is bound to the scalar value. Examples: -
	...
	// skipped if an old value could not be found. You can opt a transition rule
	// into unconditional evaluation by setting `optionalOldSelf` to true.
	"rule"!: string
}
