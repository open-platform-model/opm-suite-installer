...
#CronJobSpec: {
	...
	// validation and the controller manager during execution. If no system-wide
	// time zone database can be found a bundled version of the database is used
	// instead. If the time zone name becomes invalid during the lifetime of a
	// CronJob or due to a change in host configuration, the controller will stop
	// creating new new Jobs and will create a system event with the reason
...
#JobSpec: {
	...
	// 6, unless backoffLimitPerIndex (only Indexed Job) is specified. When
	...
	// this index as failed. When enabled the number of failures per index is kept
	// in the pod's batch.kubernetes.io/job-index-failure-count annotation. It can
	// only be set when Job's completionMode=Indexed, and the Pod's restart policy
	// is Never. The field is immutable.
	...
	// of failed pods, represented by the jobs's .status.failed field, is
	// incremented and it is checked against the backoffLimit. This field cannot be
	// used in combination with restartPolicy=OnFailure.
	...
	// TerminatingOrFailed and Failed are allowed values when podFailurePolicy is
	// not in use.
	...
	// only when the number of succeeded pods equals to the completions. When the
	// field is specified, it must be immutable and works only for the Indexed
	// Jobs. Once the Job meets the SuccessPolicy, the lingering pods are
	// terminated.
	"successPolicy"?: #SuccessPolicy

	// suspend specifies whether the Job controller should create Pods or not. If a
...
#JobStatus: {
	...
	// happens-before order across separate operations. It is represented in
	// RFC3339 form and is in UTC. The completion time is set when the job finishes
	// successfully, and only then. The value cannot be updated or removed. The
	...
	// Job is suspended, one of the conditions will have type "Suspended" and
	...
	// conditions. The "Complete", "Failed" and "FailureTarget" conditions cannot
	// be disabled.
	//
	...
	// commas. The numbers are listed in increasing order. Three or more
	...
	// indexes are 1, 3, 4, 5 and 7, they are represented as "1,3-5,7". The set of
	// failed indexes cannot overlap with the set of completed indexes.
	...
	// Represents time when the job controller started processing a job. When a Job
	// is created in the suspended state, this field is not set until the first
	// time it is resumed. This field is reset every time a Job is resumed from
	...
	// The number of pods which reached phase Succeeded. The value increases
	// monotonically for a given spec. However, it may decrease in reaction to
	// scale down of elastic indexed jobs.
	"succeeded"?: int32 & int
...
// a failed pod based on its container exit codes. In particular, it lookups
// the .state.terminated.exitCode for each app container and init container
// status, represented by the .status.containerStatuses and
...
#PodFailurePolicyOnExitCodesRequirement: {
	...
	// When null, the rule applies to all containers. When specified, it should
	...
	// values with respect to the operator. The list of values must be ordered and
	// must not contain duplicates. Value '0' cannot be used for the In operator.
...
#PodFailurePolicyRule: {
	...
	// satisfied if at least one pattern matches an actual pod condition. At most
...
#SuccessPolicy: {
	...
	// rules are met, the "SuccessCriteriaMet" condition is added, and the
...
#SuccessPolicyRule: {
	...
	// "1-4", succeededCount is "3", and completed indexes are "1", "3", and "5",
	// the Job isn't declared as succeeded because only "1" and "3" indexes are
	// considered in that rules. When this field is null, this doesn't default to
	// any value and is never evaluated at any time. When specified it needs to be
	// a positive integer.
	"succeededCount"?: int32 & int

	// succeededIndexes specifies the set of indexes which need to be contained in
	// the actual set of the succeeded indexes for the Job. The list of indexes
	// must be within 0 to ".spec.completions-1" and must not contain duplicates.
	// At least one element is required. The indexes are represented as intervals
	// separated by commas. The intervals can be a decimal integer or a pair of
	// decimal integers separated by a hyphen. The number are listed in represented
	// by the first and last element of the series, separated by a hyphen. For
	// example, if the completed indexes are 1, 3, 4, 5 and 7, they are represented
	// as "1,3-5,7". When this field is null, this field doesn't default to any
	// value and is never evaluated at any time.
