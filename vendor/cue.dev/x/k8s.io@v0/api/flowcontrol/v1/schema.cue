...
#ExemptPriorityLevelConfiguration: {
	// `lendablePercent` prescribes the fraction of the level's NominalCL that can
	// be borrowed by other priority levels. This value of this field must be
	// between 0 and 100, inclusive, and it defaults to 0. The number of seats that
	// other levels can borrow from this level, known as this level's
	...
	// limit (ServerCL) is divided among all the priority levels in proportion to
	// their NCS values:
	//
...
#LimitedPriorityLevelConfiguration: {
	...
	// known as this level's BorrowingConcurrencyLimit (BorrowingCL) and is a limit
	// on the total number of seats that this level may borrow at any one time.
	// This field holds the ratio of that limit to the level's nominal concurrency
	// limit. When this field is non-nil, it must hold a non-negative integer and
	...
	// `lendablePercent` prescribes the fraction of the level's NominalCL that can
	// be borrowed by other priority levels. The value of this field must be
	// between 0 and 100, inclusive, and it defaults to 0. The number of seats that
	// other levels can borrow from this level, known as this level's
	// LendableConcurrencyLimit (LendableCL), is defined as follows.
	//
...
#PolicyRulesWithSubjects: {
	...
	// includes both the system:authenticated and system:unauthenticated user
...
#PriorityLevelConfigurationSpec: {
	...
	// priority level are not subject to a limit (and thus are never queued) and do
	// not detract from the capacity made available to other priority levels. A
...
#QueuingConfiguration: {
	...
	// request's flow identifier (a string pair) is hashed and the hash value is
	// used to shuffle the list of queues and deal a hand of the size specified
	...
	// `queueLengthLimit` is the maximum number of requests allowed to be waiting in
	// a given queue of this priority level at a time; excess requests are
	...
	// effectively precludes shufflesharding and thus makes the distinguisher
	// method of associated flow schemas irrelevant. This field has a default value
	// of 64.
	"queues"?: int32 & int
}
...
// either (d1) the request does not specify a namespace (i.e., `Namespace==""`)
