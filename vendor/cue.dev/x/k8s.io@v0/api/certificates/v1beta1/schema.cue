...
#ClusterTrustBundleSpec: {
	...
	// trustBundle contains the individual X.509 trust anchors for this bundle, as
...
#PodCertificateRequestSpec: {
	...
	// the message field.
	//
	// Deprecated: This field is replaced by StubPKCS10Request. If StubPKCS10Request
	// is set, this field must be empty. Signer implementations should extract the
	// public key from the StubPKCS10Request field.
	...
	// pod UID, using RSASSA-PSS from RFC 8017 (as implemented by the golang
	...
	//
	// Deprecated: This field is replaced by StubPKCS10Request. If StubPKCS10Request
	// is set, this field must be empty.
	...
	// the Kubernetes project, `kubernetes.io/kube-apiserver-client-pod`, which
	// will issue client certificates understood by kube-apiserver. It is currently
	// unimplemented.
	"signerName"!: string

	// A PKCS#10 certificate signing request (DER-serialized) generated by Kubelet
...
#PodCertificateRequestStatus: {
	...
	// refresh the certificate. This field is set via the /status subresource, and
	// must be set at the same time as certificateChain. Once populated, this field
	// is immutable.
	//
	// This field is only a hint. Kubelet may start refreshing before or after this time if necessary.
	"beginRefreshAt"?: v1.#Time

	// certificateChain is populated with an issued certificate by the signer. This
	...
	// a pod volume, kubelet will drop any data in-between the PEM blocks, as well
	// as any PEM block headers.
	...
	// notAfter is the time at which the certificate expires. The value must be the
	...
	// notBefore is the time at which the certificate becomes valid. The value must
	...
	// populated, it is immutable. The signer must set this field at the same time
	// it sets certificateChain.
	"notBefore"?: v1.#Time
}
