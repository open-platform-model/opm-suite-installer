...
#CSIDriver: {
	...
	// GetPluginName() call for that driver. The driver name must be 63 characters
...
#CSIDriverSpec: {
	...
	// when the attach operation is complete. If the value is specified to false,
	...
	// nodeAllocatableUpdatePeriodSeconds specifies the interval between periodic
	...
	"nodeAllocatableUpdatePeriodSeconds"?: int64 & int
	...
	// driver registration information via CSINode object. They must create
	// simulated CSINode objects in addition to Node objects during scheduling
	// simulation, otherwise if PreventPodSchedulingIfMissing is enabled globally
	// for CSIDriver object, any newly created node may be rejected by the
	// scheduler because of missing CSI driver information from the node.
	//
	...
	// driver can be mounted separately with different `-o context` options. This
	// is typical for storage backends that provide volumes as filesystems on block
	...
	// like tokens, which is the appropriate mechanism for handling credentials.
	// This addresses security concerns where sensitive tokens were being logged as
	// part of volume context.
	//
	// When "true", kubelet will pass the tokens only in the Secrets field with the
	// key "csi.storage.k8s.io/serviceAccount.tokens". The CSI driver must be
	// updated to read tokens from the Secrets field instead of VolumeContext.
	//
	...
	// tokenRequests indicates the CSI driver needs pods' service account tokens it
	...
	// is empty string. To receive a new token after expiry, RequiresRepublish can
	// be used to trigger NodePublishVolume periodically.
...
#CSINodeDriver: {
	...
	// that the storage system will understand, e.g. "nodeA" instead of "node1".
	...
	// pass back to the driver. It is possible for different nodes to use different
...
#CSIStorageCapacity: {
	...
	//
	// This is defined since CSI spec 1.4.0 as the largest size that may be used in
	// a CreateVolumeRequest.capacity_range.required_bytes field to create a volume
	...
	// conflicts with other CSI drivers on the cluster, the recommendation is to
	// use csisc-<uuid>, a generated name, or a reverse-domain name which ends with
	// the unique CSI driver name.
	//
	...
	// exists, the CSIStorageCapacity object is obsolete and should be removed by
	// its creator. This field is immutable.
...
#StorageClass: {
	...
	"allowedTopologies"?: [...v1_9.#TopologySelectorTerm]
...
// only PersistentVolumes can be attached via external attacher, in the future
...
#VolumeAttachmentSource: {
	...
	// feature.
	"inlineVolumeSpec"?: v1_9.#PersistentVolumeSpec
...
#VolumeAttachmentStatus: {
	...
	// attached indicates the volume is successfully attached. This field must only
...
#VolumeAttributesClass: {
	...
	// underlying storage provider supports changing these attributes on an
	// existing volume, however the parameters field itself is immutable. To invoke
	// a volume update, a new VolumeAttributesClass should be created with new
	// parameters, and the PersistentVolumeClaim should be updated to reference the
	...
	// cumulative max size of 256K. If the CSI driver rejects invalid parameters,
...
#VolumeNodeResources: {
	...
	// applies for a unique volume that is shared among multiple pods on the same
	// node. If this field is not specified, then the supported number of volumes
	// on this node is unbounded.
	"count"?: int32 & int
}
