...
#AllocatedDeviceStatus: {
	// Conditions contains the latest observation of the device's state. If the
	// device has been configured according to the class and claim config
	// references, the `Ready` condition should be True.
	//
	...
	// This name together with the driver name and the device name field identify
...
#AllocationResult: {
	...
	"allocationTimestamp"?: v1.#Time

	// Devices is the result of allocating devices.
	"devices"?: #DeviceAllocationResult

	// NodeSelector defines where the allocated resources are available. If unset,
	// they are available everywhere.
	"nodeSelector"?: v1_9.#NodeSelector
}

// CELDeviceSelector contains a CEL expression for selecting a device.
#CELDeviceSelector: {
	// Expression is a CEL expression which evaluates a single device. It must
	// evaluate to true when the device under consideration satisfies the desired
	// criteria, and false when it does not. Any other result is an error and
	// causes allocation of devices to abort.
	//
	...
	// simplify expressions that access multiple attributes with the same domain.
	// For example:
	//
	// cel.bind(dra, device.attributes["dra.example.com"], dra.someBool && dra.anotherBool)
	//
	// When the DRAListTypeAttributes feature gate is enabled, the includes() helper
	// is available and it can work for both scalar and list-type attributes. It
	// was introduced to support smooth migration from scalar attributes to
	// list-type attributes while keeping CEL expressions simple. For example:
	//
	...
	// evaluating it is also limited based on the estimated number of logical
	// steps.
	"expression"!: string
}
...
#CapacityRequestPolicy: {
	// Default specifies how much of this capacity is consumed by a request that
	// does not contain an entry for it in DeviceRequest's Capacity.
...
#CapacityRequirements: {
	// Requests represent individual device resource requests for distinct
	// resources, all of which must be provided by the device.
	//
	// This value is used as an additional filtering condition against the available
	// capacity on the device. This is semantically equivalent to a CEL selector
	// with `device.capacity[<domain>].<name>.compareTo(quantity(<request
	// quantity>)) >= 0`. For example,
	...
	// the nearest valid value based on the policy. If the requested amount cannot
	// be adjusted to a valid value—because it exceeds what the requestPolicy
	// allows— the device is considered ineligible for allocation.
	//
	...
	// requests must not exceed the capacity value. The consumed capacity, which
	// may be adjusted based on the requestPolicy if defined, is recorded in the
	// resource claim’s status.devices[*].consumedCapacity field.
...
#Device: {
	...
	//
	// Must only be set if Spec.PerDeviceNodeSelection is set to true. At most one
	// of NodeName, NodeSelector and AllNodes can be set.
	"allNodes"?: bool
	...
	"attributes"?: [string]: #DeviceAttribute

	// BindingConditions defines the conditions for proceeding with binding. All of
	// these conditions must be set in the per-device status conditions with a
	// value of True to proceed with binding the pod to the node while scheduling
	// the pod.
	//
	...
	// BindingFailureConditions defines the conditions for binding failure. They may
	// be set in the per-device status conditions. If any is set to "True", a
	...
	// BindsToNode indicates if the usage of an allocation involving this device has
	...
	//
	// The maximum number of attributes and capacities combined is 32.
	"capacity"?: [string]: #DeviceCapacity

	// ConsumesCounters defines a list of references to sharedCounters and the set
	// of counters that the device will consume from those counter sets.
	//
	...
	// Name is unique identifier among all devices managed by the driver in the
	// pool. It must be a DNS label.
	"name"!: string

	// NodeAllocatableResourceMappings defines the mapping of node resources that
	// are managed by the DRA driver exposing this device. This includes resources
	...
	// addition to standard requests made through the Pod `spec`, these resources
	// can also be requested through claims and allocated by the DRA driver. For
	// example, a CPU DRA driver might allocate exclusive CPUs or auxiliary node
	// memory dependencies of an accelerator device. The keys of this map are the
	// node-allocatable resource names (e.g., "cpu", "memory"). Extended resource
	// names are not permitted as keys.
	"nodeAllocatableResourceMappings"?: [string]: #NodeAllocatableResourceMapping
	...
	// ResourceSlice, then the maximum number of allowed devices per ResourceSlice
...
#DeviceAllocationConfiguration: {
	...
	// Requests lists the names of requests where the configuration applies. If
	// empty, its applies to all requests.
	//
	// References to subrequests must include the name of the main request and may
	// include the subrequest using the format <main request>[/<subrequest>]. If
	// just the main request is given, the configuration applies to all
	// subrequests.
	"requests"?: [...string]
...
#DeviceAllocationResult: {
	// This field is a combination of all the claim and class configuration
...
#DeviceAttribute: {
	...
	// VersionValue is a semantic version according to semver.org spec 2.0.0. Must
	// not be longer than 64 characters.
	"version"?: string

	// VersionValues is a non-empty list of semantic versions according to
	// semver.org spec 2.0.0. Each version string must not be longer than 64
	// characters.
	//
	// This is an alpha field and requires enabling the DRAListTypeAttributes feature gate.
	"versions"?: [...string]
...
#DeviceCapacity: {
	// RequestPolicy defines how this DeviceCapacity must be consumed when the
	// device is allowed to be shared by multiple allocations.
	//
	// The Device must have allowMultipleAllocations set to true in order to set a requestPolicy.
	//
	// If unset, capacity requests are unconstrained: requests can consume any
	// amount of capacity, as long as the total consumed across all allocations
	// does not exceed the device's defined capacity. If request is also unset,
	// default is the full capacity value.
	"requestPolicy"?: #CapacityRequestPolicy

	// Value defines how much of a certain capacity that device has.
	//
	// This field reflects the fixed total capacity and does not change. The
	// consumed amount is tracked separately by scheduler and does not affect this
	// value.
	"value"!: resource.#Quantity
}
...
#DeviceClassSpec: {
	...
	// claimed via this class. Some classses may potentially be satisfied by
	// multiple drivers, so each instance of a vendor configuration applies to
	// exactly one driver.
	//
...
#DeviceConstraint: {
	// DistinctAttribute requires that all devices in question have this attribute
	...
	// attributes must be pairwise disjoint across devices. Scalar values are
	// treated as singleton sets for backward compatibility.
	//
	...
	// MatchAttribute requires that all devices in question have this attribute and
	...
	// which does not have that attribute will not be chosen. All devices should
	// use a value of the same type for this attribute because that is part of its
	// specification, but if one device doesn't, then it also will not be chosen.
	//
	// When the DRAListTypeAttributes feature gate is enabled, comparison uses set
	// semantics(i.e., element order and duplicates are ignored): list-valued
	// attributes match when the intersection across all devices is non-empty.
	// Scalar values are treated as singleton sets for backward compatibility.
	//
	// Must include the domain qualifier.
	"matchAttribute"?: string

	// Requests is a list of the one or more requests in this claim which must
	// co-satisfy this constraint. If a request is fulfilled by multiple devices,
	// then all of the devices must satisfy the constraint. If this is not
	// specified, this constraint applies to all requests in this claim.
	//
	// References to subrequests must include the name of the main request and may
	// include the subrequest using the format <main request>[/<subrequest>]. If
	// just the main request is given, the constraint applies to all subrequests.
	"requests"?: [...string]
}
...
// DeviceRequest is a request for devices required for a claim. This is
// typically a request for a single resource like a device, but can also ask
// for several identical devices. With FirstAvailable it is also possible to
// provide a prioritized list of requests.
#DeviceRequest: {
	...
	// the scheduler. It tries to satisfy them in the order in which they are
	// listed here. So if there are two entries in the list, the scheduler will
	// only check the second one if it determines that the first one can not be
	// used.
	//
	// DRA does not yet implement scoring, so the scheduler will select the first
	// set of devices that satisfies all the requests in the claim. And if the
	// requirements can be satisfied on more than one node, other scheduling
	// features will determine which node is chosen. This means that the set of
	// devices allocated to a claim might not be the optimal set available to the
	// cluster. Scoring will be implemented later.
	"firstAvailable"?: [...#DeviceSubRequest]

	// Name can be used to reference this request in a
	// pod.spec.containers[].resources.claims entry and in a constraint of the
	// claim.
	//
	// References using the name in the DeviceRequest will uniquely identify a
	// request when the Exactly field is set. When the FirstAvailable field is set,
	// a reference to the name of the DeviceRequest will match whatever subrequest
	// is chosen by the scheduler.
	//
	// Must be a DNS label.
	"name"!: string
}
...
#DeviceRequestAllocationResult: {
	...
	// Admin access is disabled if this field is unset or set to false, otherwise
	// it is enabled.
	"adminAccess"?: bool

	// BindingConditions contains a copy of the BindingConditions from the
	...
	// BindingFailureConditions contains a copy of the BindingFailureConditions from
	...
	// ConsumedCapacity tracks the amount of capacity consumed per device as part of
	...
	// requestPolicy if applicable (i.e., may not be less than the requested
	// amount).
	//
	...
	"consumedCapacity"?: [string]: resource.#Quantity
	...
	// Request is the name of the request in the claim which caused this device to
	// be allocated. If it references a subrequest in the firstAvailable list on a
	...
	// ShareID uniquely identifies an individual allocation share of the device,
	// used when the device supports multiple simultaneous allocations. It serves
	// as an additional map key to differentiate concurrent shares of the same
	// device.
	"shareID"?: string
...
#DeviceSubRequest: {
	...
	// default value will be applied to each capacity according to requestPolicy.
	// For the capacity that has no requestPolicy, default is the full capacity
	// value.
	//
	// Applies to each device allocation. If Count > 1, the request fails if there
	// aren't enough devices that meet the requirements. If AllocationMode is set
	// to All, the request fails if there are devices that otherwise match the
	// request, and have this capacity, with a value >= the requested amount, but
	// which cannot be allocated to this request.
	"capacity"?: #CapacityRequirements

	// Count is used only when the count mode is "ExactCount". Must be greater than
	...
	// free to request anything without restrictions, then administrators can
	// create an empty DeviceClass for users to reference.
	"deviceClassName"!: string

	// Name can be used to reference this subrequest in the list of constraints or
	// the list of configurations for the claim. References must use the format
	// <main request>/<subrequest>.
	//
	// Must be a DNS label.
	"name"!: string

	// Selectors define criteria which must be satisfied by a specific device in
	// order for that device to be considered for this subrequest. All selectors
	// must be satisfied for a device to be considered.
	"selectors"?: [...#DeviceSelector]

	// If specified, the request's tolerations.
	//
	// Tolerations for NoSchedule are required to allocate a device which has a
	// taint with that effect. The same applies to NoExecute.
	//
	// In addition, should any of the allocated devices get tainted with NoExecute
	// after allocation and that effect is not tolerated, then all pods consuming
	// the ResourceClaim get deleted to evict them. The scheduler will not let new
	// pods reserve the claim while it has these tainted devices. Once all pods are
...
// The ResourceClaim this DeviceToleration is attached to tolerates any taint
...
#DeviceToleration: {
	...
	// the system. If larger than zero, the time when the pod needs to be evicted
	// is calculated as <time when taint was adedd> + <toleration seconds>.
	"tolerationSeconds"?: int64 & int

	// Value is the taint value the toleration matches to. If the operator is
	// Exists, the value must be empty, otherwise just a regular string. Must be a
	// label value.
	"value"?: string
}
...
#ExactDeviceRequest: {
	// AdminAccess indicates that this is a claim for administrative access to the
	...
	// Selectors define criteria which must be satisfied by a specific device in
	// order for that device to be considered for this request. All selectors must
...
#NodeAllocatableResourceMapping: {
	// AllocationMultiplier is used as a multiplier for the allocated device count
	// or the allocated capacity in the claim. It defaults to 1 if not specified.
	// How the field is used also depends on whether `capacityKey` is set. 1. If
	...
	// multiplied with the base quantity. For example, if
	// `spec.devices[*].capacity` has an entry "dra.example.com/memory": "128Gi",
	// and this field is set to "dra.example.com/memory", then for a claim
	// allocation that consumes { "dra.example.com/memory": "4Gi" } the base
	// quantity for the node allocatable resource mapping will be "4Gi", and
	// `allocationMultiplier` should be omitted or set to "1".
	"capacityKey"?: string
}
...
// use by workloads. For example, if a workload needs an accelerator device
...
#ResourceClaimConsumerReference: {
	...
	// core API. This matches the group in the APIVersion that is used when
...
#ResourceClaimStatus: {
	...
	// Devices contains the status of each device allocated for this claim, as
	// reported by the driver. This can include driver-specific information.
	...
	// A Pod which references a ResourceClaim which is not reserved for that Pod
	// will not be started. A claim that is in use or might be in use because it
	// has been reserved must not get deallocated.
	//
	...
	// ResourceClaim which already has reached its maximum number of consumers,
...
#ResourcePool: {
	...
	// pools which are comprised of multiple ResourceSlices and are in an
	// incomplete state.
...
#ResourceSliceSpec: {
	...
	// PerDeviceNodeSelection defines whether the access from nodes to resources in
	// the pool is set on the ResourceSlice level or on each device. If it is set
	// to true, every device defined the ResourceSlice must specify this
	// individually.
	//
	// Exactly one of NodeName, NodeSelector, AllNodes, and PerDeviceNodeSelection must be set.
