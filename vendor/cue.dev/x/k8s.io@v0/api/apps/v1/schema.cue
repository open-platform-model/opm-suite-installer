...
#DeploymentStatus: {
	...
	// Total number of non-terminating pods targeted by this deployment (their
	// labels match the selector).
	"replicas"?: int32 & int

	// Total number of terminating pods targeted by this deployment. Terminating
	// pods have a non-null .metadata.deletionTimestamp and have not yet reached
	...
	// Total number of non-terminating pods targeted by this deployment that have
...
#ReplicaSet: {
	...
	// the Pod(s) that the ReplicaSet manages. Standard object's metadata. More
	// info:
	// https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
	"metadata"?: v1.#ObjectMeta
...
#ReplicaSetStatus: {
	...
	// The number of non-terminating pods that have labels matching the labels of
...
#RollingUpdateDaemonSet: {
	...
	// available (Ready for at least minReadySeconds) the old DaemonSet pod on that
	// node is marked deleted. If the old pod becomes unavailable for any reason
	// (Ready transitions to false, is evicted, or is drained) an updated pod is
	// immediately created on that node without considering surge limits. Allowing
	// surge implies the possibility that the resources consumed by the daemonset
	// on any given node can double if the readiness check fails, and so resource
	// intensive daemonsets should take into account that they may cause evictions
	// during disruption.
	...
	// that at least 70% of original number of DaemonSet pods are available at all
...
#RollingUpdateStatefulSetStrategy: {
	...
	// be 0. Defaults to 1. This field is beta-level and is enabled by default. The
	// field applies to all pods in the range 0 to Replicas-1. That means if there
	// is any unavailable pod in the range 0 to Replicas-1, it will be counted
	// towards MaxUnavailable. This setting might not be effective for the
	// OrderedReady podManagementPolicy. That policy ensures pods are created and
	// become ready one at a time.
	"maxUnavailable"?: intstr.#IntOrString
