...
#ApplyConfiguration: {
	// expression will be evaluated by CEL to create an apply configuration. ref:
	...
	// risk of accidental deletion of values not included in the apply
	// configuration.
	//
	...
	//
	// CEL expressions have access to the contents of the API request, organized
	// into CEL variables as well as some other useful variables:
	//
	...
	// checks for the principal (user or service account) of the request.
...
#AuditAnnotation: {
	...
	// key will be identical. In this case, the first annotation written with the
	// key will be included in the audit event and all subsequent annotations with
	// the same key will be discarded.
	//
	// Required.
	"key"!: string

	// valueExpression represents the expression which is evaluated by CEL to
	// produce an audit annotation value. The expression must evaluate to either a
	// string or null value. If the expression evaluates to a string, the audit
	// annotation is included with the string value. If the expression evaluates to
	// null or empty string the audit annotation will be omitted. The
	// valueExpression may be no longer than 5kb in length. If the result of the
	...
	// then the valueExpression will be evaluated for each binding. All unique
	// values produced by the valueExpressions will be joined together in a
...
#ExpressionWarning: {
	// fieldRef is the path to the field that refers to the expression. For example,
	// the reference to the expression of the first item of validations is
	// "spec.validations[0].expression"
	"fieldRef"!: string

	// warning contains the content of type checking information in a human-readable
...
#MatchResources: {
	...
	// or extensions/v1beta1 would not be sent to the ValidatingAdmissionPolicy.
	//
	...
	// object based on whether the namespace for that object matches the selector.
...
// MutatingAdmissionPolicy describes the definition of an admission mutation
...
// MutatingAdmissionPolicyBindingSpec defines the specification of the
// MutatingAdmissionPolicyBinding.
#MutatingAdmissionPolicyBindingSpec: {
	// matchResources limits what resources match this binding and may be mutated by
	// it. Note that if matchResources matches a resource, the resource must also
	// match a policy's matchConstraints and matchConditions before the resource
	// may be mutated. When matchResources is unset, it does not constrain resource
	...
	// matchResources.resourceRules are optional and do not constraint matching
	// when unset. Note that this is differs from MutatingAdmissionPolicy
	...
	// spec.ParamKind of the bound MutatingAdmissionPolicy. If the policy specifies
...
#MutatingAdmissionPolicySpec: {
	...
	// validated. Match conditions filter requests that have already been matched
	// by the matchConstraints. An empty list of matchConditions matches all
	...
	// validate. The MutatingAdmissionPolicy cares about a request if it matches
	// _all_ Constraints. However, in order to prevent clusters from being put into
	...
	// MutatingAdmissionPolicyBinding. The CREATE, UPDATE and CONNECT operations
	// are allowed. The DELETE operation may not be matched. '*' matches CREATE,
	// UPDATE and CONNECT. Required.
	"matchConstraints"?: #MatchResources

	// mutations contain operations to perform on matching objects. mutations may
	// not be empty; a minimum of one mutation is required. mutations are evaluated
	// in order, and are reinvoked according to the reinvocationPolicy. The
	// mutations of a policy are invoked for each binding of this policy and
	// reinvocation of mutations occurs on a per binding basis.
	"mutations"?: [...#Mutation]

	// paramKind specifies the kind of resources used to parameterize this policy.
	// If absent, there are no parameters for this policy and the param CEL
	// variable will not be provided to validation expressions. If paramKind refers
	// to a non-existent kind, this policy definition is mis-configured and the
...
#MutatingWebhook: {
	...
	// matchConditions is a list of conditions that must be met for a request to be
	// sent to this webhook. Match conditions filter requests that have already
	// been matched by the rules, namespaceSelector, and objectSelector. An empty
	...
	// name is the name of the admission webhook. Name should be fully qualified,
	...
	// option *must* be idempotent, able to process objects they previously
	// admitted. Note: * the number of additional invocations is not guaranteed to
	// be exactly one. * if additional invocations result in further modifications
	// to the object, webhooks are not guaranteed to be invoked again. * webhooks
	// that use this option may be reordered to minimize the number of additional
	// invocations. * to validate an object after all mutations are guaranteed
...
#Mutation: {
	...
	// jsonPatch defines a [JSON patch](https://jsonpatch.com/) operation to perform
...
#NamedRuleWithOperations: {
	...
	// operations that are added. If '*' is present, the length of the slice must
	// be one. Required.
	"operations"?: [...string]
...
#ParamRef: {
	...
	// care that if this is left empty the binding must not match any
	...
	// parameters will be treated as successful validation by the binding. If set
	// to `Deny`, then no matched parameters will be subject to the `failurePolicy`
	// of the policy.
	//
...
#ValidatingAdmissionPolicy: {
	...
	// status represents the current status of the ValidatingAdmissionPolicy,
...
#ValidatingAdmissionPolicyBindingSpec: {
	...
	// ParamKind then this field is ignored, and the rules are evaluated without a
	// param.
	"paramRef"?: #ParamRef

	// policyName references a ValidatingAdmissionPolicy name which the
	// ValidatingAdmissionPolicyBinding binds to. If the referenced resource does
	// not exist, this binding is considered invalid and will be ignored Required.
	"policyName"!: string

	// validationActions declares how Validations of the referenced
	// ValidatingAdmissionPolicy are enforced. If a validation evaluates to false
	// it is always enforced according to these actions.
	//
	// Failures defined by the ValidatingAdmissionPolicy's FailurePolicy are
	// enforced according to these actions only if the FailurePolicy is set to
	// Fail, otherwise the failures are ignored. This includes compilation errors,
	...
	// a JSON list of objects, each with the following fields: - message: The
...
#ValidatingAdmissionPolicySpec: {
	...
	// runtime errors and invalid or mis-configured policy definitions or bindings.
	//
	// A policy is invalid if spec.paramKind refers to a non-existent Kind. A
	// binding is invalid if spec.paramRef.name refers to a non-existent resource.
	//
	// failurePolicy does not define how validations that evaluate to false are handled.
	//
	...
	// matchConditions is a list of conditions that must be met for a request to be
	// validated. Match conditions filter requests that have already been matched
	// by the rules, namespaceSelector, and objectSelector. An empty list of
...
#Validation: {
	...
	// contents of the API request/response, organized into CEL variables as well
	// as some other useful variables:
	//
	// - 'object' - The object from the incoming request. The value is null for
	// DELETE requests. - 'oldObject' - The existing object. The value is null for
	// CREATE requests. - 'request' - Attributes of the API
	...
	// accessible. Accessible property names are escaped according to the following
	// rules when accessed in the expression: - '__' escapes to '__underscores__' -
	// '.' escapes to '__dot__' - '-' escapes to '__dash__' - '/' escapes to
	...
	// contain line breaks. If unset, the message is "failed rule: {Rule}". e.g.
	// "must be a URL with the host matching spec.host" If the Expression contains
	// line breaks. Message is required. The message must not contain line breaks.
	// If unset, the message is "failed Expression: {Expression}".
	"message"?: string

	// messageExpression declares a CEL expression that evaluates to the validation
	// failure message that is returned when this rule fails. Since
	// messageExpression is used as a failure message, it must evaluate to a
	// string. If both message and messageExpression are present on a validation,
	// then messageExpression will be used if validation fails. If
	// messageExpression results in a runtime error, the runtime error is logged,
	// and the validation failure message is produced as if the messageExpression
	// field were unset. If messageExpression evaluates to an empty string, a
	// string with only spaces, or a string that contains line breaks, then the
	// validation failure message will also be produced as if the messageExpression
	...
	// to the client. The currently supported reasons are: "Unauthorized",
...
#Variable: {
	...
	// unique among all variables. The variable can be accessed in other
	// expressions through `variables` For example, if name is "foo", the variable
...
#WebhookClientConfig: {
	...
	// apiservers (e.g., `kube-apiserver` cannot resolve in-cluster DNS as that
