...
#PodDisruptionBudgetSpec: {
	...
	// null selector will match no pods, while an empty ({}) selector will select
...
#PodDisruptionBudgetStatus: {
	...
	"currentHealthy"?: int32 & int

	// minimum desired number of healthy pods
	"desiredHealthy"?: int32 & int
	...
	// when the API server processed the eviction request to the time when the pod
	// is seen by PDB controller as having been marked for deletion (or after a
	// timeout). The key in the map is the name of the pod and the value is the
	// time when the API server processed the eviction request. If the deletion
	// didn't occur and a pod is still there it will be removed from the list
	// automatically by PodDisruptionBudget controller after some time. If
	// everything goes smooth this map should be empty for the most of the time.
	// Large number of entries in the map may indicate problems with pod deletions.
	"disruptedPods"?: [string]: v1.#Time

	// Number of pod disruptions that are currently allowed.
	"disruptionsAllowed"?: int32 & int

	// total number of pods counted by this disruption budget
	"expectedPods"?: int32 & int
