...
// example: http://<host>/<path>?<searchpart> -> backend where where parts of
// the url correspond to RFC 3986, this resource will be used to match against
...
#IngressBackend: {
	...
	// with "Service".
	"resource"?: v1_9.#TypedLocalObjectReference
...
#NetworkPolicySpec: {
	...
	// selector matches all pods in the policy's namespace. Multiple network
	// policies can select the same set of pods. In this case, the ingress rules
	// for each are combined additively. This field is optional. If it is not
	// specified, it defaults to an empty selector.
	"podSelector"?: v1.#LabelSelector
	...
	// is not specified, it will default based on the existence of ingress or
	...
	// to write a policy that specifies that no egress is allowed, you must specify
	// a policyTypes value that include "Egress" (since such a policy would not
...
#ServiceCIDRSpec: {
	...
	// "2001:db8::/64") from which to assign service cluster IPs. Max of two CIDRs
	// is allowed, one of each IP family. This field is immutable.
	"cidrs"?: [...string]
}
