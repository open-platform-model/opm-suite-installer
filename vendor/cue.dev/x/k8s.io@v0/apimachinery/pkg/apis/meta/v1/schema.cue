...
#APIGroup: {
	...
	// preferredVersion is the version preferred by the API server, which probably
	...
	// match. In case of multiple matches, clients should use the longest matching
	// CIDR. The server returns only those CIDRs that it thinks that the client can
	// match. For example: the master will return an internal IP CIDR only, if the
	// client reaches the server using an internal IP. Server looks at
	// X-Forwarded-For header or X-Real-Ip header or request.RemoteAddr (in that
...
#APIResource: {
	...
	// The hash value of the storage version, the version this resource is converted
	// to when written to the data store. Value must be treated as opaque by
	// clients. Only equality comparison on the value is valid. This is an alpha
	// feature and may change or be removed in the future. The field is populated
	// by the apiserver only if the StorageVersionHash feature gate is enabled.
	...
	// version is the preferred version of the resource. Empty implies the version
	// of the containing resource list For subresources, this may have a different
	// value, for example: v1 (while inside a v1beta1 version of the core
	// resource's group)".
	"version"?: string
}
...
#DeleteOptions: {
	...
	// immediately.
	"gracePeriodSeconds"?: int64 & int

	// if set to true, it will trigger an unsafe deletion of the resource in case
	// the normal deletion flow fails with a corrupt object error. A resource is
...
#ListMeta: {
	...
	// omitted during serialization. Servers older than v1.15 do not set this
	// field. The intended use of the remainingItemCount is *estimating* the size
	// of a collection. Clients should not rely on the remainingItemCount to be set
	// or to be exact.
	"remainingItemCount"?: int64 & int
...
#ManagedFieldsEntry: {
	...
	// related to the Subresource field and it always corresponds to the version of
	// the main resource.
	"subresource"?: string

	// Time is the timestamp of when the ManagedFields entry was added. The
	// timestamp will also be updated if a field is added, the manager changes any
...
#ObjectMeta: {
	...
	"deletionGracePeriodSeconds"?: int64 & int
	...
	// reorder it. If the finalizer list is processed in order, then this can lead
	// to a situation in which the component responsible for the first finalizer in
	// the list is waiting for a signal (field value, external system, or other)
	// produced by a component responsible for a finalizer later in the list,
	// resulting in a deadlock. Without enforced ordering finalizers are free to
	// order amongst themselves and are not vulnerable to ordering changes in the
	// list.
	"finalizers"?: [...string]
...
// subset.
#ShardInfo: {
	// selector is the shard selector string from the request, echoed back so
	// clients can verify which shard they received and merge responses from
	// multiple shards.
	"selector"!: string
}
...
// Status object defines what attributes will be set. Clients must ignore
