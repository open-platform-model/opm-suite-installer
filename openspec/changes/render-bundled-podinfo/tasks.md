...
- [ ] 2.1 Add `task vendor:sync`, populating `vendor/` with `core`, both catalogs and
...
- [ ] 2.2 Create `modules/podinfo/` from `cli/tests/fixtures/modules/podinfo`, re-identified to
...
- [ ] 2.3 Create `platform/` as a `#Platform` module pinning core and both catalogs, modelled on
...
- [ ] 2.4 Copy the bundle, modules, platform and vendor tree into the image at the D7 paths and
...
- [ ] 3.1 Write `scripts/opm-suite` with `set -euo pipefail`, the D7 usage text, `list`, the
...
- [ ] 3.2 Add `render`: copy `/opt/opm` to a working directory under `TMPDIR`, write the
...
- [ ] 4.1 Add `task image:render` running the image with `--network=none` and an empty cache,
...
- [ ] 5.1 Add the `FINDINGS.md` entries this change earned: a fully vendored bundle renders with
  ...
  operator-owned.
- [ ] 5.2 Update `README.md` and `CLAUDE.md` for the delivered layout, `vendor/` and the
