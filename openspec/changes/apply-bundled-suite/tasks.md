...
- [x] 1.1 `task cluster:up`, then from the built image (not the host) run `opm operator install
...
- [x] 2.1 Add the in-cluster kubeconfig synthesis to `scripts/opm-suite` (D1): written under
...
- [x] 2.2 Add the `apply` subcommand: shared selection and validation, CRD bootstrap with exit
...
- [x] 2.3 Add the readiness wait on `opm instance status`, `OPM_SUITE_TIMEOUT`, and exit `73`
...
- [x] 3.1 Write `deploy/`: Namespace `opm-suite`, ServiceAccount, the D5 ClusterRole and Role
  ...
  cluster.
- [x] 3.2 Measure RBAC: start from the D5 table, run the Job, remove every rule the run did not
...
- [x] 3.3 Verify the second-run scenarios: `task job:run` again exits `0` with the same pods
  running (compare pod UIDs before and after), and a hand-edited replica count is set back by
  the re-run.

## 4. Record and close

- [x] 4.1 Add the `FINDINGS.md` entries: `opm` has no in-cluster kubeconfig fallback and how
...
- [x] 4.2 Update `README.md` and `CLAUDE.md` for `apply`, `deploy/` and the `job:*` tasks, then
