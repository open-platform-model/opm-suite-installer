...
- [x] 1.1 With podinfo applied by `apply-bundled-suite` on the `opm-suite` kind cluster, read
...
- [x] 1.2 Add `jq` to the `Containerfile` alongside `wget`, `SUITE_VERSION` into
...
- [x] 2.1 Add the capture step to `apply` (D1, D4): CR read into `$WORK/<app>.previous.json`,
  ...
  pod restarted.
- [x] 2.2 Add the hook runner (D3) and `bundle/instances/podinfo/pre-apply`; extend `task lint`
...
- [x] 2.3 Add the revert (D2, D4): on failed upgrade, `on-failure` if present, then
...
- [x] 2.4 Verify the happy upgrade: build an image with a newer real podinfo tag, run it over
...
- [x] 3.1 Add the `FINDINGS.md` entries: `opm` records `spec.values` but nothing prints it
...
- [x] 3.2 Update `README.md` and `CLAUDE.md` (hooks, exit codes, the "an upgrade is a values
